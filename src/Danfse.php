...
class Danfse extends DaCommon
{
    ...
    private function renderCabecalho($logo = null)
    {
        ...
        // Logo
        //$logo = 'uploads/configEmitente/2ZdWsTHd0IUh5f3Sa1uisgunB.png';
        ...
        if (!empty($this->logomarca)) {
            ...
            $this->pdf->Image($this->logomarca, $x + 1, $y + 1, $nImgW, $nImgH, 'jpeg');
        } else {
            $x1 = $x;
            $y1 = round($h / 3 + $y, 0);
            $tw = $w;
...
//        $this->pdf->setTextColor(0, 0, 0);
        ...
        QRCode::text('https://www.nfse.gov.br/ConsultaPublica/?tpc=1&chave=' . $idNfse)
            ->setOutfile($path)
            ->png();

        $this->pdf->Image($path, $x + 175, $y + 5, $nImgW -5, $nImgH, 'png');
...
//        $this->pdf->cell($qrSize, 3, 'portal nacional da NFS-e', 0, 0, 'C');
    ...
    private function renderPrestador()
    {
        ...
        $this->pdf->cell($w4, 3, Str::lower($this->prestador['email']) ?? '', 0, 0, 'L');
